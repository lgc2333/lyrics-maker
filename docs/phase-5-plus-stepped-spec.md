...
- 按 Esc 或点击列表空白位置可以取消当前选择，在时轴模式或歌词模式下都如此
- 歌词模式下，只有在没有一行歌词的情况下切词栏才展示导入提示（旧提示需要变了，现在的操作改了），如果行内有歌词则显示选择一行歌词以编辑
...
- 增加歌曲播放速度选项（此状态不要持久化保存）：25%，50%，75%，100%
- 播放速度控制放在音乐音量按钮左侧。
